...
const LEVEL_BG: Record<LogEntry["level"], string> = {
  INFO:    "bg-zinc-700/60 text-zinc-200",
    ...
    <div className="glass-panel rounded-2xl p-6 transition-all hover:border-zinc-600/80 group">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-mono font-bold text-sm text-white tracking-widest uppercase">{label}</span>
        </div>
        <span className="font-mono text-[10px] text-zinc-400">{subtitle}</span>
      </div>
      {children}
    </div>
  );
...
export default function AccountsPage() {
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh] bg-zinc-950">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-mono text-sm text-zinc-400 text-pretty">Đang khởi động hệ thống...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-hyper-modern bg-grainy text-zinc-200 flex flex-col font-sans selection:bg-zinc-800 selection:text-white">
      {/* ── HEADER NAVIGATION ── */}
      <header className="sticky top-0 z-50 bg-hyper-modern bg-grainy/80 backdrop-blur-xl border-b border-white/5 px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-sm tracking-tight text-white">AutoFarm Simulator</span>
            <span className="px-2 py-0.5 rounded-full bg-zinc-800/50 text-[10px] font-mono text-zinc-300 border border-white/5 uppercase">v3.5</span>
          </div>

          <nav className="flex items-center gap-1">
            <button 
              onClick={() => setActiveTab("global")}
              className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${activeTab === "global" ? "bg-white/10 text-white" : "text-zinc-400 hover:text-zinc-200 hover:bg-white/5"}`}
            >
              Cấu Hình Chung
            </button>
            <button 
              onClick={() => setActiveTab("fb")}
              className={`px-4 py-2 text-sm font-medium rounded-lg transition-all flex items-center gap-2 ${activeTab === "fb" ? "bg-white/10 text-white" : "text-zinc-400 hover:text-zinc-200 hover:bg-white/5"}`}
            >
              Facebook Engine
            </button>
            <button 
              onClick={() => setActiveTab("threads")}
              className={`px-4 py-2 text-sm font-medium rounded-lg transition-all flex items-center gap-2 ${activeTab === "threads" ? "bg-white/10 text-white" : "text-zinc-400 hover:text-zinc-200 hover:bg-white/5"}`}
            >
              Threads Engine
            </button>
          </nav>
        ...
        <div className="flex items-center gap-4">
          <div className="flex flex-col items-end">
            <span className="text-[11px] font-medium text-zinc-300">{userEmail}</span>
          ...
          <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-zinc-800 to-zinc-700 flex items-center justify-center border border-white/10">
            <span className="text-xs font-bold text-zinc-200">{userEmail?.charAt(0).toUpperCase()}</span>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-[1200px] w-full mx-auto p-8">
        ...
        {activeTab === "global" && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
            <div>
              <h1 className="text-fluid-h1 font-semibold text-white tracking-tight mb-2 text-balance">Cấu Hình Chung</h1>
              <p className="text-sm text-zinc-400 text-pretty">Quản lý kho link Affiliate và nhận thông báo qua Telegram.</p>
            </div>

            <div className="page-layout">
              <div className="area-half shadow-layered glass-panel p-6 hover:border-white/10 transition-colors">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-medium text-white">Affiliate Link Pool</h3>
                ...
                <textarea 
                  ...
                  className="w-full bg-hyper-modern bg-grainy border border-white/5 rounded-xl p-4 text-[13px] font-mono text-zinc-200 placeholder:text-zinc-500 focus:outline-none focus:border-zinc-500 focus:ring-1 focus:ring-zinc-500/50 resize-none transition-all mb-4" 
                />
                <button 
              ...
              <div className="area-half shadow-layered glass-panel p-6 hover:border-white/10 transition-colors h-fit">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-medium text-white">Telegram Notify</h3>
                ...
                <input 
                  ...
                  className="w-full bg-hyper-modern bg-grainy border border-white/5 rounded-xl p-4 text-[13px] font-mono text-zinc-200 placeholder:text-zinc-500 focus:outline-none focus:border-zinc-500 focus:ring-1 focus:ring-zinc-500/50 transition-all" 
                />
            ...
            {parsedLinks.length > 0 && (
              <div className="glass-panel p-6">
                <h3 className="font-medium text-white mb-6">AI Parsing Results ({parsedLinks.length})</h3>
                  ...
                  {parsedLinks.map((p, i) => (
                    <div key={i} className="flex gap-4 bg-hyper-modern bg-grainy border border-white/5 p-4 rounded-xl">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-white truncate mb-1">{p.title}</p>
                        <p className="text-[11px] text-zinc-400 font-mono truncate mb-2">{p.aff_link}</p>
                        <p className="text-xs text-zinc-300 italic">"{p.suggested_comment}"</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {activeTab === "fb" && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 page-layout">
            <div className="area-half space-y-6">
              <div>
                <h1 className="text-fluid-h1 font-semibold text-white tracking-tight mb-2 text-balance">Facebook Engine</h1>
                <p className="text-sm text-zinc-400 text-pretty">Thiết lập Cookie và chạy tiến trình Facebook.</p>
              </div>

              <div className="glass-panel p-6">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-medium text-white">FB Access Cookie</h3>
                ...
                <textarea 
                  ...
                  className="w-full bg-hyper-modern bg-grainy border border-white/5 rounded-xl p-4 text-[13px] font-mono text-emerald-400 font-bold placeholder:text-zinc-700 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20 resize-none transition-all mb-6" 
                />
            ...
            <div className="area-half shadow-layered h-[500px] bg-[#0c0c0c] border border-white/5 rounded-2xl flex flex-col overflow-hidden">
              <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between bg-[#111]">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-mono font-medium text-zinc-200">fb-live-logs.log</span>
                </div>
                <button onClick={() => setFbLogs([{ time: now(), level: "INFO", msg: "Console cleared." }])} className="text-zinc-500 hover:text-zinc-200 transition-colors">
        ...
        {activeTab === "threads" && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
            <div className="page-layout items-start">
              <div className="area-half space-y-6">
                <div>
                  <h1 className="text-fluid-h1 font-semibold text-white tracking-tight mb-2 text-balance">Threads Engine</h1>
                  <p className="text-sm text-zinc-400 text-pretty">Cấu hình Bot Threads và chỉnh sửa bài đăng.</p>
                </div>

                <div className="glass-panel p-6">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="font-medium text-white">Threads Access Cookie</h3>
                  ...
                  <textarea 
                    ...
                    className="w-full bg-hyper-modern bg-grainy border border-white/5 rounded-xl p-4 text-[13px] font-mono text-blue-400 font-bold placeholder:text-zinc-700 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/20 resize-none transition-all mb-6" 
                  />
                  <button 
                ...
                <div className="h-[350px] bg-[#0c0c0c] border border-white/5 rounded-2xl flex flex-col overflow-hidden">
                  <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between bg-[#111]">
                    <div className="flex items-center gap-2">
                      <Terminal className="w-4 h-4 text-zinc-300" />
                      <span className="text-xs font-mono font-medium text-zinc-200">threads-live-logs.log</span>
                    </div>
                    <button onClick={() => setThreadsLogs([{ time: now(), level: "INFO", msg: "Console cleared." }])} className="text-zinc-500 hover:text-zinc-200 transition-colors">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                  <div className="flex-1 overflow-y-auto p-4 space-y-2 font-mono text-[12px] custom-scrollbar">
              ...
              <div className="area-half shadow-layered glass-panel p-6 flex flex-col h-[700px]">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-medium text-white">Threads Crawl Poster</h3>
                  <span className="px-3 py-1 bg-zinc-800 rounded-full text-xs text-zinc-300">{threadsPosts.length} Bài Đăng</span>
                </div>
                ...
                <div className="flex-1 overflow-y-auto pr-2 space-y-4 custom-scrollbar dim-siblings">
                  {threadsPosts.map((post, i) => (
                    <div key={post.id} className="bg-hyper-modern bg-grainy border border-white/5 rounded-xl p-5 relative group/post transition-colors hover:border-zinc-700">
                      ...
                      <textarea 
                        className="w-full bg-transparent text-[13px] text-zinc-200 resize-none outline-none leading-relaxed min-h-[100px]"
                      ...
                      <div className="mt-6 flex justify-end gap-3">
                        <button onClick={() => handleSavePost(post)} className="btn-premium bg-zinc-800 text-zinc-200 hover:bg-zinc-700 text-xs font-medium px-4 py-2 rounded-lg transition-all">
                          Lưu
                        </button>
